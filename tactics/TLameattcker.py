...
class TLameattacker(Tactic):
    ...
    def getState(self,state):
        ...
        if k1 or k2 :
            return TLameattacker.State.shoot
        ...
        if ball_in_possession :
            return TLameattacker.State.cross_pass
        ...
        if fabs(ballPos.dist(Vector2D(int(state.homePos[self.bot_id].x),int(state.homePos[self.bot_id].y)))>1200 and ball_in_possession and pass_possible):
            return TLameattacker.State.cross_receive
        ...
        if fabs(ballPos.dist(Vector2D(int(state.homePos[self.bot_id].x),int(state.homePos[self.bot_id].y)))>1200 and ball_in_possession and not pass_possible):
            return TLameattacker.State.optimum_pos 
    ...
    def execute(self, state , pub):
        ...
        if gameState == TLameattacker.State.shoot:
        ...
        if gameState == TLameattacker.State.cross_pass:
        ...
        if gameState == TLameattacker.State.cross_receive:
        ...
        if gameState == TLameattacker.State.optimum_pos:
